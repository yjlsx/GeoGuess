...
export type ImageInputCopy = {
  assetCountUnit: string;
  clearFile: string;
  fileInputLabel: string;
  fileSizeFallback: string;
  hint: string;
  maxSize: string;
  notesLabel: string;
...
const defaultCopy: ImageInputCopy = {
  assetCountUnit: "个素材",
  clearFile: "清除文件",
  fileInputLabel: "上传图片",
...
export function ImageInput({ copy = defaultCopy, files, displayAssetName = null, notes, onFileChange, onNotesChange }: Props) {
  ...
  return (
    <section className={displayedAssetName ? "panel workflow-panel image-input-panel has-asset" : "panel workflow-panel image-input-panel"}>
      ...
      <label className="upload-dropzone">
        <span className="sr-only">{copy.fileInputLabel}</span>
        <input
        ...
        <strong>{copy.uploadCta}</strong>
        <small>{copy.supportedFormats}<br />{copy.maxSize}</small>
      </label>
      {displayedAssetName ? (
        <div className="asset-file-row">
          ...
          <div>
            ...
            <span>
              {firstFile ? `${files.length} ${copy.assetCountUnit} / ${(firstFile.size / 1024 / 1024).toFixed(1)} MB` : copy.fileSizeFallback}
            </span>
          </div>
          {firstFile ? (
            <button type="button" aria-label={copy.clearFile} onClick={() => onFileChange([])}>×</button>
          ) : (
            <span className="asset-row-static-close" aria-hidden="true">×</span>
          )}
        </div>
      ) : null}
      {files.length === 0 ? <p className="hint">{copy.hint}</p> : null}
      <label className="field">
        {copy.notesLabel}
        <textarea value={notes} onChange={(event) => onNotesChange(event.target.value)} placeholder={copy.notesPlaceholder} />
      </label>
    </section>
  );
}
